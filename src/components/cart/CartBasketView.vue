<template>
  <transition name="fade-slide">
    <div v-if="isVisible" @click.self="toggleCart" class="fixed inset-0 bg-black bg-opacity-50 flex justify-end">
      <!-- Overlay + toggleCart -->
      <div @click.stop class="cart-content bg-[#181818] w-96 h-full p-4 overflow-y-auto"> <!-- Cart content & .stop -->
        <!-- top close X -->
        <button @click="toggleCart"
          class="absolute top-3 right-4 text-4xl text-red-400 hover:text-red-700">&times;</button> <!-- Close button -->

        <h2 class="text-2xl font-bold mb-4">Cart</h2>
        <div v-for="item in cart" :key="item._id" class="mb-4"> <!-- Loop through the cart items -->

          <div class="flex pb-2">
            <img :src="item.imageURL" alt="Product Image" class="h-24 w-28 object-cover rounded-lg">
            <!-- Product image -->
        ...
        <div class="pt-4 border-t ">
          ...
          <input type="text" class="border p-1 pr-2 bg-[#181818] text-right w-28" placeholder="Enter code"
            v-model="discountCode">
          <!-- Coupon code -->
          <p class="text-right font-semibold">Grand Total: $ {{ grandTotal() }} </p> <!-- Grand total in the cart -->
          <div class="flex justify-end">
            <button class="bg-green-600 text-white p-2 mt-4 rounded hover:bg-green-700">Checkout</button>
...
const { cart, updateQuantity, discountCode, cartTotal, cartTotalIndividualProduct, salesTax, grandTotal } = useCart()
